...
def afficher():
    ...
    for i, (nom, numero, adresse, mois, montant, date) in enumerate(records, start=1):
        tree.insert("", "end", values=(nom, numero, adresse, mois, montant, date))
...
def rechercher():
    if (e2.get() == ""):
        ...
        con = pymysql.connect(host="localhost", user="root", password="sidi", database="gardiennage")
        cursor = con.cursor()
        cursor.execute("SELECT * FROM paiement WHERE numero ='" + e2.get() + "'")
...
def ajouter():
    ...
    if (
            nom == "" or numero == "" or adresse == "" or mois == "" or montant == "" or date == "" ):
        ...
        cursor = con.cursor();
        cursor.execute(
            "insert into paiement values('" + nom + "','" + numero + "','" + adresse + "','" + mois + "','" + montant + "','" + date + "') ")
...
def modifier():
    ...
    if (nom == "" or numero == "" or adresse == "" or mois == "" or montant == "" or date == "" ):
        messagebox.showinfo("Message d'erreur", "Aucun champ ne doit être vide !!!")
    else:
        con = pymysql.connect(host="localhost", user="root", password="sidi", database="gardiennage")
        cursor = con.cursor();
        cursor.execute("update paiement set nom='" + nom + "',numero='" + numero + "',adresse='" + adresse + "',mois='" + mois + "',montant='" + montant + "',date='" + date +"' where nom='" + nom + "'")
        cursor.execute("commit");
...
label = tk.Label(master=root,text="Paiement",bg="#46887C",fg='#ffffff' ,font=("regular",18))
...
e4 = ttk.Combobox(root,values = ['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'],width=20,height=100,font=(30))
e4.place(x=550,y=90)
e4.current()
# entry = customtkinter.CTkEntry(master=root,width=200,height=30,fg_color="#ffffff",corner_radius=8)
# entry.place(x=650,y=100, anchor=tkinter.CENTER)
...
e6 = tkcalendar.DateEntry(root,bd=2,font=("Times New Roman",12),date_pattern = "YYYY-MM-DD")
e6.place(x=550,y=190,width=200)
...
tree.heading(2, text='Numéro')
tree.heading(3, text='Adresse')
tree.heading(4, text='Mois')
...
# Label(root,text="Numéro",fg="#000000",bg='white',font=('regular',15,)).place(x=50,y=130)
...
# Label(root,text="Montant",fg="#000000",bg='white',font=('regular',15,)).place(x=450,y=80)
