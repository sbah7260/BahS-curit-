from tkinter import *
from tkinter import ttk
import tkinter as tk
import customtkinter as customtkinter
from tkinter.messagebox import showinfo
import tkinter
from subprocess import call
import pymysql as pymysql
from tkinter import messagebox
import tkcalendar
root=Tk()
root.geometry("800x550+200+100")
root.iconbitmap("22926756.ico")
# root.config(background="#FFFFFF")
# root.resizable(False,False)
root.title("Salaire")
def Retour():
    root.destroy()
    call(["python","acc.py"])
# define columns

tree = ttk.Treeview(root, columns=(1,2,3,4,5,6,7,8),show='headings')

#connection à la base de doonées

def afficher():
    con = pymysql.connect(host="localhost", user="root", password="sidi", database="gardiennage")
    cursor = con.cursor()
    cursor.execute("select * from salaire ")
    tree.delete(*tree.get_children())
    records = cursor.fetchall()
    print(records)

    for i, (nom, prenom, adresse, mode_agent, numero, mois, montant,date) in enumerate(records, start=1):
        tree.insert("", "end", values=(nom, prenom, adresse, mode_agent, numero, mois, montant,date))

    con.close()


afficher()


def rechercher():
    if (e5.get() == ""):
        messagebox.showinfo("Recherche ", "Veuillez entrer le numéro de l'agent à rechercher !!!")
    else:
        con = pymysql.connect(host="localhost", user="root", password="sidi", database="gardiennage")
        cursor = con.cursor()
        cursor.execute("SELECT * FROM salaire WHERE numero ='" + e5.get() + "'")
        rows = cursor.fetchall()

        for row in rows:
            e1.insert(0, row[0])
            e2.insert(0, row[1])
            e3.insert(0, row[2])
            e4.insert(0, row[3])
            e6.insert(0, row[5])
            e7.insert(0, row[6])
            e8.delete(0, "end")
            e8.insert(0, row[7])

        con.close();


def ajouter():
    nom = e1.get()
    prenom = e2.get()
    adresse = e3.get()
    mode_agent = e4.get()
    numero = e5.get()
    mois = e6.get()
    montant = e7.get()
    date = e8.get()


    if (
            nom == "" or prenom == "" or adresse == "" or mode_agent == "" or numero == "" or mois == "" or montant == "" or date == ""):
        messagebox.showinfo("Message d'erreur", "Tout les champs sont réquis !!!")
    else:

        con = pymysql.connect(host="localhost", user="root", password="sidi", database="gardiennage")
        cursor = con.cursor();
        cursor.execute(
            "insert into salaire values('" + nom + "','" + prenom + "','" + adresse + "','" + mode_agent + "','" + numero + "','" + mois + "','" + montant + "','" + date + "') ")
        cursor.execute("commit");

        e1.delete(0, "end")
        e2.delete(0, "end")
        e3.delete(0, "end")
        e4.delete(0, "end")
        e5.delete(0, "end")
        e6.delete(0, "end")
        e7.delete(0, "end")
        e8.delete(0, "end")

        messagebox.showinfo("Ajouter ", "L'ajout effectuer avec succès")
    afficher()

    con.close()


def modifier():
    nom = e1.get()
    prenom = e2.get()
    adresse = e3.get()
    mode_agent = e4.get()
    numero = e5.get()
    mois = e6.get()
    montant = e7.get()
    date = e8.get()


    if (nom == "" or prenom == "" or adresse == "" or mode_agent == "" or numero == "" or mois == "" or montant == "" or date == ""):
        messagebox.showinfo("Message d'erreur", "Aucun champ ne doit être vide !!!")
    else:
        con = pymysql.connect(host="localhost", user="root", password="sidi", database="gardiennage")
        cursor = con.cursor();
        cursor.execute("update salaire set nom='" + nom + "',prenom='" + prenom + "',adresse='" + adresse + "',mode_agent='" + mode_agent + "',numero='" + numero + "',mois='" + mois + "',montant='" + montant + "',date='" + date + "' where nom='" + nom + "'")
        cursor.execute("commit");

        e1.delete(0, "end")
        e2.delete(0, "end")
        e3.delete(0, "end")
        e4.delete(0, "end")
        e5.delete(0, "end")
        e6.delete(0, "end")
        e7.delete(0, "end")
        e8.delete(0, "end")

        messagebox.showinfo("Modification ", "Modification effectuer avec succès")

        afficher()

        con.close()

frame =Frame(master=root,bg="#46887C",width=2800,height=70,)
frame.place(relx=0, rely=0, anchor=tkinter.CENTER)
label = tk.Label(master=root,text="Salaire",bg="#46887C",fg='#ffffff' ,font=("regular",18))
# label.place(x=400, y=17, anchor=tkinter.CENTER)
label.pack(padx=100, pady=0)
button = Button(root, text="Retour", fg='#ffffff',bg='#000000',font=("regular",12),command=Retour)
button.place(x=0,y=0)

nom = customtkinter.CTkLabel(master=root,text="Nom",text_font=('regular',15,),corner_radius=8)
nom.place(x=42, y=100, anchor=tkinter.CENTER)
prenom = customtkinter.CTkLabel(master=root,text="Prénom",text_font=('regular',15,),corner_radius=8)
prenom.place(x=50, y=150, anchor=tkinter.CENTER)
adresse = customtkinter.CTkLabel(master=root,text="Adresse",text_font=('regular',15,),corner_radius=8)
adresse.place(x=55, y=200, anchor=tkinter.CENTER)
mode_agent = customtkinter.CTkLabel(master=root,text="Mode agent",text_font=('regular',15,),corner_radius=8)
mode_agent.place(x=70, y=250, anchor=tkinter.CENTER)
numero = customtkinter.CTkLabel(master=root,text="Numéro",text_font=('regular',15,), corner_radius=8)
numero.place(x=450, y=100, anchor=tkinter.CENTER)
mois = customtkinter.CTkLabel(master=root,text="Mois",text_font=('regular',15,),corner_radius=8)
mois.place(x=435, y=150, anchor=tkinter.CENTER)
montant = customtkinter.CTkLabel(master=root,text="Montant",text_font=('regular',15,),corner_radius=8)
montant.place(x=450, y=200, anchor=tkinter.CENTER)
date = customtkinter.CTkLabel(master=root, text="Date", text_font=('regular', 15,), corner_radius=8)
date.place(x=435, y=250, anchor=tkinter.CENTER)

e1 = customtkinter.CTkEntry(master=root,width=200,height=30,fg_color="#ffffff",corner_radius=8)
e1.place(x=250,y=100, anchor=tkinter.CENTER)
e2 = customtkinter.CTkEntry(master=root,width=200,height=30,fg_color="#ffffff",corner_radius=8)
e2.place(x=250,y=150, anchor=tkinter.CENTER)
e3 = customtkinter.CTkEntry(master=root,width=200,height=30,fg_color="#ffffff",corner_radius=8)
e3.place(x=250,y=200, anchor=tkinter.CENTER)
e4 = ttk.Combobox(root,values = ['7H-18H','18H-7H','24H/24H'],width=20,height=100,font=(30))
e4.place(x=150,y=230)
e4.current()

e5 = customtkinter.CTkEntry(master=root,width=200,height=30,fg_color="#ffffff",corner_radius=8)
e5.place(x=650,y=100, anchor=tkinter.CENTER)
e6 = customtkinter.CTkEntry(master=root,width=200,height=30,fg_color="#ffffff",corner_radius=8)
e6.place(x=650,y=200, anchor=tkinter.CENTER)
e7 = ttk.Combobox(root,values = ['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'],width=20,height=100,font=(30))
# pprint(dict(comboExample))
e7.place(x=550,y=140)
e7.current()
# e8 = customtkinter.CTkEntry(master=root, width=200, height=30, fg_color="#ffffff", corner_radius=8)
# e8.place(x=550, y=240)
e8 = tkcalendar.DateEntry(root,bd=2,font=("Times New Roman",12),date_pattern = "YYYY-MM-DD")
e8.place(x=550,y=230,width=200)

style = ttk.Style(root)
style.theme_use("clam")
style.configure("Treeview", background="gray90", fieldbackground="white",highlightthickness=0, bd=0, foreground="black")
style.configure("mystyle.Treeview.Heading", font=('Calibri', 8,'bold'))


# define headings
tree.heading(1, text='Nom')
tree.heading(2, text='Prénom')
tree.heading(3, text='Adresse')
tree.heading(4, text='Mode agent')
tree.heading(5, text='Numéro')
tree.heading(6, text='Mois')
tree.heading(7, text='Montant')
tree.heading(8, text='Date')
tree.column(1,width=15)
tree.column(2,width=15)
tree.column(3,width=15)
tree.column(4,width=15)
tree.column(5,width=15)
tree.column(6,width=15)
tree.column(7,width=15)
tree.column(8,width=15)

def item_selected(event):
    for selected_item in tree.selection():
        item = tree.item(selected_item)
        record = item['values']
        # show a message
        showinfo(title='Information', message=','.join(record))


tree.bind('<<TreeviewSelect>>', item_selected)
tree.place(x=0,y=320,width=780,height=220)
# tree.place(x=0,y=280, sticky='nsew')

# add a scrollbar
scrollbar = ttk.Scrollbar(root, orient=tk.VERTICAL, command=tree.yview)
tree.configure(yscroll=scrollbar.set)
scrollbar.place(x=780,y=320,height=220)


button = customtkinter.CTkButton(master=root, text="Ajouter", fg_color=("#4F805D", "#4F805D"),text_color='#ffffff',text_font=("regular",15),command=ajouter)
button.place(x=80,y=280)
button = customtkinter.CTkButton(master=root, text="Modifier", fg_color=("#58A8A3", "#58A8A3"),text_color='#ffffff',text_font=("regular",15),command=modifier)
button.place(x=270,y=280)
button = customtkinter.CTkButton(master=root, text="Rechercher", fg_color=("#455785", "#455785"),text_color='#ffffff',text_font=("regular",15),command=rechercher)
button.place(x=450,y=280)



root.mainloop()

# # from sqlalchemy import create_engine
# from tkinter import ttk
# import tkinter as tk
# import sqlite3
# # my_con = sqlite3.connect('my_db.db')
# # r_set = my_con.execute("SELECT count(*) as no from STUDENT")
#
# columns = ('nom','date','email')
# tree = ttk.Treeview(root, columns = columns  , show='headings')
# tree.heading('nom', text = 'Nom')
# tree.heading('date', text = 'Date')
# tree.heading('email', text = 'Email')
# tree.grid(row=0, column=0, sticky='nsew')
#
# data_row = tree.set()
# no_rec=data_row[0]
# limit =8
# root = tk.Tk()
# def my_display(offset):
#
#     back = offset - limit
#     next = offset + limit
#     b1 = tk.Button(root, text='Next >', command=lambda: my_display((next)))
#     b1.grid(row=12,column=4)
#     b2 = tk.Button(root, text='< Prev', command=lambda: my_display((back)))
#     b2.grid(row=12, column=4)
#     if(no_rec <= next):
#         b1["state"]="disabled"
#     else:
#         b1["state"]="active"
#
#     if(back>=0):
#         b2["state"]="active"
#     else:
#         b2["state"]="disabled"
# my_display(0)

# button = Button(root, text="Ajouter", fg='#000000',bg='#4F805D',font=("regular",15))
# button.place(x=100,y=280)
# button = Button(root, text="Modifier", fg='#000000',bg='#58A8A3',font=("regular",15))
# button.place(x=300,y=280)
# button = Button(root, text="Rechercher", fg='#000000',bg='#455785',font=("regular",15))
# button.place(x=500,y=280)
# Label(root,text="Nom",fg="#000000",bg='white',font=('regular',15,)).place(x=50,y=80)
# Label(root,text="Prénom",fg="#000000",bg='white',font=('regular',15,)).place(x=50,y=130)
# Label(root,text="Adresse",fg="#000000",bg='white',font=('regular',15,)).place(x=50,y=180)
# Label(root,text="Mode d'agent",fg="#000000",bg='white',font=('regular',15,)).place(x=50,y=230)
# Label(root,text="Numéro",fg="#000000",bg='white',font=('regular',15,)).place(x=450,y=80)
# Label(root,text="Mois",fg="#000000",bg='white',font=('regular',15,)).place(x=450,y=180)
# Label(root,text="Montant",fg="#000000",bg='white',font=('regular',15,)).place(x=450,y=130)
# e2 = Entry(root,width=25,font=(30),border=3)
# e2.place(x=200,y=85)
# e3 = Entry(root,width=25,font=(30),border=3)
# e3.place(x=200,y=130)
# e4 = Entry(root,width=25,font=(30),border=3)
# e4.place(x=200,y=180)
# e4 = Entry(root,width=25,font=(30),border=3)
# e4.place(x=200,y=230)